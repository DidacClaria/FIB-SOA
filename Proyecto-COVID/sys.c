...
void* sys_sbrk(int incr){
  void* base_ptr=current()->heap_ptr;
  page_table_entry *current_PT = get_PT(current());
  int num_pag, new_pag, page_count=0;
  if (incr==0) return base_ptr;
  else if (incr>0){
    if (((int)base_ptr+incr)/PAGE_SIZE<TOTAL_PAGES && ((int)base_ptr%PAGE_SIZE==0 || incr>=PAGE_SIZE || (int)base_ptr%PAGE_SIZE>=((int)base_ptr+incr)%PAGE_SIZE)){
        //RESERVAR PAGINAS I INCREMENTAR HEAP
        num_pag=(incr+4096-1)/4096; //to round up the number of pages
        while (page_count<num_pag){
          new_pag=alloc_frame();
          if (new_pag!=-1){
            set_ss_pag(current_PT, ((int)base_ptr+4096-1)/PAGE_SIZE+page_count, new_pag);
          }
          else {
